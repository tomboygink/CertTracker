    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    db_res = _a.sent();
                    if (!(db_res.rows[0] !== undefined)) return [3, 3];
                    return [4, this.transporter.sendMail({
                            from: config.config_mail.auth.user,
                            to: this.args.email,
                            subject: "Восстановление пароля",
                            html: 'Данное письмо отправлено для сброса пароля на сайте программного обеспечения "Система СДС". <h1><a href="http://' +
                                config.server_config.host +
                                ":" +
                                config.server_config.port +
                                "/reset_pass?code= " +
                                db_res.rows[0].re_password_code + " mail=" + this.args.email +
                                ' ">Нажмите на данное сообщение для сброса пароля </a></h1>',
                        })];
                case 2:
                    _a.sent();
                    return [2, db_res.rows[0]];
